#tag Class
 Attributes ( "@Version" = "1.2.2", "@Copyright" = "(c) 2022-2025 watermann-it.de", "@Guid" = "7391BC4F-5E17-4EED-A7FC-7D000EF67BA5", "@Author" = "Stefan Watermann", "@Description" = "Base-class used for model-classes to convert to and from JSON. Recursive JSON Support.", "@Depends" = "-", "@History" = "Fix missing encoding for ToJson. ToJson as JSONItem. Handling MemoryBlock types. Support for #JsonIgnore attribute." ) Protected Class JsonModelBase
	...
	#tag Method, Flags = &h0
		Shared Sub FromDatabaseRow(row as DatabaseRow, o as Object)
		  Using Introspection
		  ...
		  For Each p As PropertyInfo In t.GetProperties
		    If p.IsPublic And p.CanWrite And Not p.IsComputed And Not JsonModelBase.HasAttribute(p, "#JsonIgnore") Then
		      ...
		      If data.HasKey(p.Name) And Not data.Value(p.Name).IsNull Then
		        ...
		        Case "Color"
		          p.Value(o) = data.Value(p.Name).ColorValue.ToString
		  ...
		  For Each p As PropertyInfo In t.GetProperties
		    ...
		    If Not HasAttribute(p, "#JsonIgnore") Then
		      ...
		      If p.IsPublic And p.CanRead And Not p.IsComputed and not p.Value(self).IsNull Then
		        ...
		        ElseIf p.PropertyType.FullName = "Color" Then
		          json.Value(p.Name) = p.Value(Self).ColorValue.ToString
