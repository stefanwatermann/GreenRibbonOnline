#tag Module
 Attributes ( "@Version" = "1.3.0", "@Copyright" = "(c) 2022 - watermann-it.de", "@Guid" = "91FE84EC-3D79-415A-9EE7-3E6DB1FF9150", "@Description" = "Read/Write CSV added; Read/Write lines() added; iOS Support; MoveToTrash()" ) Protected Module File
	#tag Method, Flags = &h0
	...
	#tag Method, Flags = &h0
		Sub MoveToTrash(f as folderItem)
		  Var trashFolderItem As FolderItem = SpecialFolder.Trash.Child(f.name)
		  
		  If trashFolderItem.Exists Then
		    If trashFolderItem.IsFolder Then
		      trashFolderItem.RemoveFolderAndContents
		    Else
		      trashFolderItem.Remove
		    End
		  end
